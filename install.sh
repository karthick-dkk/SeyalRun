...
INSTALL_DIR="${SEYALRUN_DIR:-/opt/seyalrun}"
...
# Run a command with privilege, non-interactively. Uses SUDO_PASSWORD when the
# environment supplies it (unattended installs on a host whose sudo needs a
# password), plain sudo otherwise (NOPASSWD or a live terminal). A no-op when
# already root.
priv() {
  if [ "$(id -u)" -eq 0 ]; then "$@"
  elif [ -n "${SUDO_PASSWORD:-}" ]; then echo "$SUDO_PASSWORD" | sudo -S "$@"
  else sudo "$@"; fi
}
...
# /opt is root-owned, so create it with privilege and hand ownership to the
# current user — every later write (.env, TLS, compose) then needs no sudo,
# and the control script can be run by the same operator.
if [[ ! -d "${INSTALL_DIR}" ]]; then
  if mkdir -p "${INSTALL_DIR}" 2>/dev/null; then :; else
    priv mkdir -p "${INSTALL_DIR}" || fail "could not create ${INSTALL_DIR} (need sudo; set SUDO_PASSWORD for an unattended run)"
  fi
fi
[[ -w "${INSTALL_DIR}" ]] || priv chown -R "$(id -un):$(id -gn)" "${INSTALL_DIR}" || fail "could not take ownership of ${INSTALL_DIR}"
cd "${INSTALL_DIR}"
...
# ── Control script ────────────────────────────────────────────────────────────
# A single entry point for the lifecycle, so an operator does not have to
# remember the compose -f flags and profile. The exact COMPOSE invocation this
# install computed is embedded, so the control script always matches how the
# stack was actually brought up (dockerized vs external DB, chosen ports).
CTL="${INSTALL_DIR}/seyalrunctl"
cat > "$CTL" <<CTLSCRIPT
#!/bin/bash
# SeyalRun lifecycle control. Generated by install.sh — do not hand-edit the
# compose flags; re-run the installer to regenerate.
set -euo pipefail
# Absolute install dir, embedded at generation time. Deriving it from
# BASH_SOURCE breaks when this is called through the /usr/local/bin symlink —
# dirname then resolves to /usr/local/bin, which has no compose files.
cd "${INSTALL_DIR}"
COMPOSE=(${COMPOSE[@]})
cmd="\${1:-}"
case "\$cmd" in
  ...
  *) echo "usage: \$0 {start|stop|restart|status|logs|down|update}"; exit 1 ;;
esac
CTLSCRIPT
chmod +x "$CTL"
# Best-effort symlink onto PATH so it can be called as 'seyalrunctl' from
# anywhere; the /opt path keeps working regardless if this cannot be created.
priv ln -sf "$CTL" /usr/local/bin/seyalrunctl 2>/dev/null || true
...
echo ""
echo "  Manage it:"
echo "    ${INSTALL_DIR}/seyalrunctl start | stop | restart | status | logs | update"
echo "    seyalrunctl status        # if /usr/local/bin is on your PATH"
echo "======================================================"
